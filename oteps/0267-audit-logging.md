...
`1`. `audit.sequence.stream_id` provides this scope as an opaque
per-`AuditLogger`-instance identifier generated by the SDK. It is a
record-level attribute rather than a `Resource` attribute because a
single OTLP batch may contain records from multiple loggers with
different stream identities.

The `audit.integrity.signer` attribute resolves the question of where
in the pipeline a signature was produced. A producer signature (`signer:
producer`) provides non-repudiation: the application attests the event
at the point of action. A collector signature (`signer: collector`)
provides custody attestation: the collector attests the record traversed
the pipeline unaltered. Both are useful for compliance; they are not
equivalent, and consumers MUST NOT treat them interchangeably.

`producer` is the preferred and default value: SDK implementations SHOULD
always set `audit.integrity.signer` to `producer` explicitly. When the
attribute is absent, consumers and collectors MUST assume `producer` —
the absence of an explicit signer does not imply collector or unknown
origin.
