...
`audit.sequence.number`, and `audit.sequence.previous_hash`. The signing algorithm
...
When `audit.sequence.number` and `audit.sequence.previous_hash` are present the
...
- `audit.sequence.number` MUST be strictly greater than the previous
  ...
  is ambiguous.
- `audit.sequence.previous_hash` MUST equal the `IntegrityHash` of the preceding
  record.
